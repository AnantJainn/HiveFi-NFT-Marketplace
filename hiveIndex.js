...
/**
 * Scans the last 1000 operations for each account in SCAN_ACCOUNTS
 * and updates our in-memory NFT list, including "likes" from HiveFi_vote ops.
 */
async function scanHiveForNFTs() {
  ...
  for (const account of SCAN_ACCOUNTS) {
    try {
      ...
      console.error(
        `Error fetching account history for ${account}:`,
        error.message
      );
    }
  }
...
/**
 * Merges new NFT ops (HiveFi_nft, HiveFi_nft_sale) or vote ops (HiveFi_vote) into our store.
 */
...
function applyVoteOp(voteData) {
  ...
  if (index !== -1) {
    ...
    console.log(
      `Incremented voteCount for NFT "${nftId}" to ${nftStore[index].voteCount}`
    );
  } else {
