...
document.getElementById("upload-btn").addEventListener("click", function () {
  ...
  document
    .getElementById("upload-version-image")
    .addEventListener("click", function () {
      optionContainer.remove();
    ...
    document
      ...
      .addEventListener("click", function () {
        ...
        const tags = tagsInput
          ...
          : []; // Trim tags to remove extra spaces
        ...
        if (title && description && tags.length > 0) {
          ...
          hive_keychain.requestCustomJson(
            ...
            function (response) {
              if (response.success) {
                ...
                console.error(
                  "Failed to store image metadata on Hive:",
                  response.message
                );
                alert(
                  "Failed to store image metadata on Hive: " + response.message
                );
              }
            }
          );
          ...
          alert(
            "Please fill in all required fields before uploading the image."
          );
        }
      });
...
function handleVersionImageUpload() {
  ...
  if (originalCid) {
    ...
    document
      .getElementById("confirm-original-yes")
      .addEventListener("click", function () {
        confirmContainer.remove();

        const newImageCid = prompt(
          "Enter the IPFS CID for the new image version:"
        );
        if (newImageCid) {
          const newTitle =
            prompt("Enter the new title (or leave blank to keep original):") ||
            "Updated Title";
          const newDescription =
            prompt(
              "Enter a new description (or leave blank to keep original):"
            ) || "Updated Description";
          const newTagsInput = prompt(
            "Enter new tags (comma-separated, or leave blank to keep original):"
          );
          const newTags = newTagsInput
            ? newTagsInput.split(",").map((tag) => tag.trim())
            : ["default_tag"]; // Trim tags
            ...
            hive_keychain.requestCustomJson(
              ...
              function (response) {
                if (response.success) {
                  ...
                  console.error(
                    "Failed to store new version on Hive:",
                    response.message
                  );
                  alert(
                    "Failed to store new version on Hive: " + response.message
                  );
                }
              }
            );
    ...
    document
      .getElementById("confirm-original-no")
      .addEventListener("click", function () {
        confirmContainer.remove();
